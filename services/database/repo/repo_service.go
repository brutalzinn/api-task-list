...
func Delete(id int64, userId string) (int64, error) {
	...
	res, err := conn.Exec(ctx, "DELETE FROM repos WHERE id=$1 and user_id=$2", id, userId)
	if err != nil {
		return 0, err
	}
...
func GetAll() (repos []database_entities.Repo, err error) {
	...
	defer conn.Close(ctx)
	rows, err := conn.Query(ctx, "SELECT * FROM repos")
	if err != nil {
		return
	}
	for rows.Next() {
...
func Count(userId string) (count int64, err error) {
	...
	defer conn.Close(ctx)
	row := conn.QueryRow(ctx, "SELECT COUNT(*) FROM repos user_id=$1", userId)
	err = row.Scan(&count)
	return
}
func Get(id int64) (repo database_entities.Repo, err error) {
	...
	defer conn.Close(ctx)
	row := conn.QueryRow(ctx, "SELECT * FROM repos WHERE id=$1", id)
...
func Insert(repo database_entities.Repo) (id int64, err error) {
	...
	err = conn.QueryRow(ctx, sql, &repo.Title, &repo.Description, &repo.UserId, time.Now()).Scan(&id)
	return
}
func Update(id int64, userId string, repo database_entities.Repo) (int64, error) {
	...
	defer conn.Close(ctx)
	res, err := conn.Exec(ctx, "UPDATE repos SET title=$1,description=$2,user_id=$3,update_at=$4 WHERE id=$5 and user_id=$6", &repo.Title, &repo.Description, userId, time.Now(), id, userId)
	if err != nil {
		return 0, err
	}
	return res.RowsAffected(), nil
